...
// Toggle filter section
    ...
    // grains And Goodness products data (fallback)
...

{
      ...
      },
      {
        ...
        // Add more grains products here as needed
    ];
    ...
    let currentPage = 1;
    let currentFilters = {
        category: 'grains', // Set to grains & goodness to filter only gg products
        subcategory: 'all',
        searchQuery: '',
        priceRange: 50,
        ratings: [],
...
const BACKEND_URL = 'https://mina-market-2.onrender.com';
    ...
    return false;
        ...
        try {
            ...
            const response = await fetch('https://mina-market-2.onrender.com/api/products/search?category=grains&limit=1', {
                signal: controller.signal
            });
...
async function fetchProductsFromMongoDB(filters, page = 1) {
  ...
  try {
    ...
    function filterLocalProducts(filters) {
        return grainsProducts.filter(product => {
            // Always filter by grains category for this page
    ...
    async function initializePage() {
        ...
        if (isMongoDBConnected) {
            ...
            populateProductsGrid(grainsProducts);
        }
    }
