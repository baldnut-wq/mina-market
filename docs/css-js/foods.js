...
// Toggle filter section
    ...
    const foodsProducts = [
      ...
      },
      {
        ...
        // Add more foods products here as needed
    ];
    ...
    let currentPage = 1;
    let currentFilters = {
        category: 'foods', // Set to foods to filter only foods products
        subcategory: 'all',
        searchQuery: '',
        priceRange: 50,
        ratings: [],
...
const BACKEND_URL = 'https://mina-market-2.onrender.com';
    ...
    return false;
        ...
        try {
            ...
            const response = await fetch('https://mina-market-2.onrender.com/api/products/search?category=foods&limit=1', {
                signal: controller.signal
            });
...
async function fetchProductsFromMongoDB(filters, page = 1) {
  ...
  try {
    ...
    function filterLocalProducts(filters) {
        return foodsProducts.filter(product => {
            // Always filter by foods category for this page
    ...
    async function initializePage() {
        ...
        if (isMongoDBConnected) {
            ...
            populateProductsGrid(foodsProducts);
        }
    }
