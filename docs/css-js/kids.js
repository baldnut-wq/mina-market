...
// Toggle filter section
    ...
    // kids play products data (fallback)
    const kidsProducts = [
        {
            id: "k1",
        ...
        image: "https://images.unsplash.com/photo-1553062407-98eeb64c6a62?ixlib=rb-4.0.3&ixid=MnwxMj",
        price: 34.99,
        originalPrice: null,
        ...
        // Add more kids products here as needed
    ];
    ...
    let currentPage = 1;
    let currentFilters = {
        category: 'kidsplay', // Set to kidsplay to filter only kids products
        subcategory: 'all',
        searchQuery: '',
        priceRange: 50,
        ratings: [],
...
const BACKEND_URL = 'https://mina-market-2.onrender.com';
    ...
    return false;
        ...
        try {
            ...
            const response = await fetch('https://mina-market-2.onrender.com/api/products/search?category=kidsplay&limit=1', {
                signal: controller.signal
            });
  ...
  try {
    ...
    function filterLocalProducts(filters) {
        return kidsProducts.filter(product => {
            // Always filter by kidsplay category for this page
            if (product.category !== 'kidsplay') {
                return false;
            }
    ...
    async function initializePage() {
        ...
        if (isMongoDBConnected) {
            ...
            populateProductsGrid(kidsProducts);
        }
    }
